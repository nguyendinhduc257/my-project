<template>
  ...
  <div class="bg-cover bg-no-repeat bg-center py-36 relative" :style="{ backgroundImage: `url('${banner}')` }">
    <div class="container">
      ...
      <p class="text-base text-gray-600 leading-6">
        Transform your living space with our curated selection of stylish and  
        <br class="hidden sm:block" />
        unique home décor items, perfect for adding a touch of elegance and 
        <br class="hidden sm:block" />
    ...
    <div class="col-span-4">
      ...
      <div v-if="cardList?.length">
        ...
        <div v-if="targetDisplayType === 'row'" class="col-span-9 mt-6 lg:mt-0 space-y-4">
          <RowCard v-for="card in cardList" :key="card.id" v-bind="card" v-bind:disableDeleteButton="true"
            v-bind:showAvailability="false" v-bind:showRating="true" v-bind:showRemoveFromWishlist="true" />
        </div>
        <div class="flex justify-center items-center mt-6">
          <RouterLink to="/product"
            class="bg-primary border border-primary text-white font-semibold hover:bg-transparent hover:text-primary transition py-2 px-4  rounded">
            More...
          </RouterLink>
        </div>
      </div>
  ...
  <div class="container py-5">
    <div class="lg:w-10/12 grid md:grid-cols-3 gap-3 lg:gap-6 mx-auto justify-center">
      <Feature v-for="feature in featureList" :key="feature.id" :image="feature.image" :name="feature.name"
  ...
  <div class="container pb-16">
    ...
    <div v-if="categoryList.length" class="grid lg:grid-cols-3 sm:grid-cols-2 gap-3">
      <Category v-for="category in categoryList" :key="category.id" :id="category.id" :name="category.name"
        :thumbnail_url="category.thumbnail_url" :slug="category.slug" />
    </div>
    <div v-else>
      Loading categories
    </div>
...
const filteredCardList = computed(() => {
  ...
  //   filtered = filtered.filter(card => card.category_id.id === selectedCategory.value)
  // }
